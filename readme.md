...
2. Using WSL2 and Docker

If you are on Windows and want to use WSL2, or if you are on Apple Silicon, it's recommended to go with option 2. 

#### Option 1: Using Vagrant and VirtualBox

Included with this project is a Vagrantfile (Ubuntu 22.04 base box) which provides a VM with the required environment and installs.
...
#### Option 2: Using WSL2 and Docker

1. Windows users only: Install WSL2 by following the official guide: https://learn.microsoft.com/en-us/windows/wsl/install. In most cases, you will only need to run `wsl --install`.
2. Install Docker Desktop.
3. Windows users only: Enable WSL2 integration in Docker settings (it's the default behavior if you've WSL2 installed).
4. Open a terminal in the project 2 folder and run: `docker compose up -d`. The /home directory is mounted, so your files inside will persist.
5. Install VS Code and the Docker extension from the marketplace.
6. Open VS Code, go to the Docker extension, find the running container, right-click, and select Attach Shell.
7. Inside the container, install extensions like C/C++ and CMake for better code navigation and debugging.
